...
class AlarmForegroundService : Service() {
    ...
    private fun showSequencerAudioNotification() {
        try {
            val notificationManager = getSystemService(NOTIFICATION_SERVICE) as NotificationManager
            
            val channelId = "sequencer_audio_channel"
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                val channel = NotificationChannel(
                    ...
                    description = "Audio playback for sequencer alarms"
            ...
            val notification = NotificationCompat.Builder(this, channelId)
                .setContentTitle("Sequencer Alarm Audio")
                .setContentText("Audio playback for multi-mission sequence")
            ...
            startForeground(SEQUENCER_AUDIO_NOTIFICATION_ID, notification)
            android.util.Log.d(TAG, "SEQUENCER_AUDIO_NOTIFICATION: Started foreground notification")
        } catch (e: Exception) {
            android.util.Log.e(TAG, "SEQUENCER_AUDIO_NOTIFICATION_ERROR: ${e.message}")
        }
    }
    
    companion object {
        private const val TAG: String = "AlarmForegroundService"
        private const val SEQUENCER_AUDIO_NOTIFICATION_ID = 7890
                    ...
                    when (alarm.missionType) {
                        "sequencer" -> {
                            ...
                            if (missionIds.isEmpty()) {
                                android.util.Log.w(TAG, "No valid missions found, skipping AlarmActivity launch")
                                return
                            }
    ...
    private fun startAudioServiceOnly() {
        try {
            android.util.Log.d(TAG, "AUDIO_ONLY_MODE: Starting audio service only")
            
            // Start only the audio playback without any mission triggers
            val currentRingtoneUri = ringtoneUri
            if (currentRingtoneUri != null) {
                startPlayback(currentRingtoneUri)
                android.util.Log.d(TAG, "AUDIO_ONLY_MODE: Audio service started successfully")
            } else {
                android.util.Log.w(TAG, "AUDIO_ONLY_MODE: No ringtone URI available")
            }
        } catch (e: Exception) {
            android.util.Log.e(TAG, "AUDIO_ONLY_MODE: Error starting audio service: ${e.message}")
        }
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val fromSequencer = intent?.getBooleanExtra("from_sequencer", false) == true
        val sequencerAudioOnly = intent?.getBooleanExtra("sequencer_audio_only", false) == true
        ...
        if (alarm != null) {
            when (alarm.missionType) {
                "sequencer" -> {
                    ...
                    // CRITICAL FIX: Block "none" missions - let sequencer handle empty cases
                    if (missionIds.isEmpty()) {
                        android.util.Log.w(TAG, "No valid missions found, using empty mission type")
                        currentMissionType = ""
                    } else {
        ...
        // Handle sequencer audio-only mode
        if (fromSequencer && sequencerAudioOnly) {
            android.util.Log.d(TAG, "SEQUENCER_AUDIO_ONLY: Starting audio playback for sequencer")
            
            // Start audio playback immediately for sequencer
            val effectiveUri = getEffectiveRingtoneUri()
            startPlayback(effectiveUri)
            
            // Show notification for sequencer audio
            showSequencerAudioNotification()
        ...
        if (isSequencerMissedAlarm) {
            ...
            try {
                ...
                if (alarm != null && alarm.missionType == "sequencer") {
                    ...
                    // Check if should trigger audio only services
                    val shouldTriggerAudioOnly = missionIds.isEmpty() || missionIds.all { it == "audio" }
                    
                    if (shouldTriggerAudioOnly) {
                        // Trigger only the audio service and nothing else
                        android.util.Log.d(TAG, "AUDIO_ONLY_MODE: Triggering only audio service for sequencer missed alarm")
                        startAudioServiceOnly()
                        return START_STICKY
                    }
