...
class MissionSequencer(private val context: Context) {
    ...
    fun startWhenAlarmFires(ringtoneUri: android.net.Uri? = null) {
        ...
        // Temporarily disabled foreground start to avoid interfering with mission UI.
        // If you need to revert, uncomment the next line.
        // startAlarmForegroundService(ringtoneUri)
        MissionLogger.log("SEQ_DEBUG: foreground start disabled (safe-mode) for alarmId=${getCurrentAlarmId()}")
                ...
                if (currentAlarmId != -1) {
                    ...
                    if (alarm != null) {
                        ...
                        val intent = Intent(context, com.vaishnava.alarm.AlarmActivity::class.java).apply {
                            ...
                            putExtra("extra_ringtone_uri", alarm.ringtoneUri?.toString())
    ...
    private fun startAlarmForegroundService(ringtoneUri: android.net.Uri? = null) {
        try {
            val currentAlarmId = getCurrentAlarmId()
            if (currentAlarmId != -1) {
                val alarmStorage = com.vaishnava.alarm.AlarmStorage(context)
                val alarm = alarmStorage.getAlarm(currentAlarmId)
                if (alarm != null) {
                    MissionLogger.log("STARTING_FOREGROUND_SERVICE: Starting AlarmForegroundService for sequencer alarm $currentAlarmId")
                    
                    val svc = Intent(context, com.vaishnava.alarm.AlarmForegroundService::class.java).apply {
                        putExtra(com.vaishnava.alarm.AlarmReceiver.ALARM_ID, alarm.id)
                        putExtra(com.vaishnava.alarm.AlarmReceiver.EXTRA_RINGTONE_URI, ringtoneUri ?: alarm.ringtoneUri)
                        putExtra("from_sequencer", true)
                        putExtra("sequencer_audio_only", true)
                    }
                    ...
                    MissionLogger.log("FOREGROUND_SERVICE_STARTED: AlarmForegroundService started for sequencer audio")
                } else {
                    MissionLogger.logError("START_FOREGROUND_SERVICE_ERROR: Alarm not found for ID $currentAlarmId")
                }
            } else {
                MissionLogger.logError("START_FOREGROUND_SERVICE_ERROR: Invalid alarm ID $currentAlarmId")
            }
        } catch (e: Exception) {
            MissionLogger.logError("START_FOREGROUND_SERVICE_EXCEPTION: ${e.message}")
        }
    }
                    ...
                    val intent = Intent(context, com.vaishnava.alarm.AlarmActivity::class.java).apply {
                        ...
                        if (currentAlarmId != -1) {
                            ...
                            ringtoneUri?.let { uri ->
                                ...
                                putExtra("extra_ringtone_uri", uri.toString())
                            }
                        }
