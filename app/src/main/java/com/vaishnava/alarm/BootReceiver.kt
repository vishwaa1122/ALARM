...
class BootReceiver : BroadcastReceiver() {
    ...
    override fun    onReceive(context: Context?, intent: Intent?) {
        ...
        // Start KeeperService if any alarms are enabled to anchor the process
        try {
            val anyEnabled = alarms.any { it.isEnabled }
            if (anyEnabled) {
                val ks = Intent(deviceProtectedContext, KeeperService::class.java)
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                    deviceProtectedContext.startForegroundService(ks)
                } else {
                    deviceProtectedContext.startService(ks)
                }
            }
        } catch (_: Exception) {
            // ignore
